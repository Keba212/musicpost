...
Ти пересилаєш аудіо боту в особистий чат. Бот зберігає Telegram `file_id` і metadata у PostgreSQL, щогодини публікує один трек із черги та додає обкладинку. За замовчуванням атмосферне фото знаходиться через Pexels, але власну картинку всередині аудіо можна вказати через `POST_IMAGE_URL`.

Щоб використовувати власну картинку як обкладинку аудіо, додай у `.env` пряме публічне HTTPS-посилання на JPG або PNG:

```text
POST_IMAGE_URL=https://example.com/my-music-cover.jpg
```

Коли `POST_IMAGE_URL` заданий, бот повторно завантажує аудіо в Telegram із цією обкладинкою. Якщо змінна порожня, бот продовжить надсилати аудіо через наявний Telegram `file_id` і випадково брати фото з Pexels.

Також можна просто надіслати картинку самому боту в приватний чат. Він збереже її як обкладинку та підтвердить це повідомленням. Цей спосіб має пріоритет над Pexels і не потребує `POST_IMAGE_URL`.
